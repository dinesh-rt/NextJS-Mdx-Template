    ...
    <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-1">
      {posts.map((post) => (
        <Link href={`/posts/${post.slug}`} key={post.slug} className="block">
          <article className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 transition-colors duration-300 hover:bg-gray-100 dark:hover:bg-gray-700 shadow-md">
            <h2 className="text-xl sm:text-2xl font-bold mb-2 text-gray-900 dark:text-white">{post.title}</h2>
