...
export async function getPostsByTag(tagSlug) {
  const filteredPosts = postsData.filter(post => post.tagSlugs && post.tagSlugs.includes(tagSlug));
...
export function getTagPostCounts() {
  ...
  tagsData.forEach(tag => {
    tagCounts[tag.slug] = postsData.filter(post => post.tagSlugs && post.tagSlugs.includes(tag.slug)).length;
  });
  return tagCounts;
}